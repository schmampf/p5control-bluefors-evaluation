...
def plot_all(
    ...
    # Check Datatye of z_data
    if z_data.dtype == np.dtype("int32"):
    ...
    # Gets proper limits for map
    z_ext, zx_lim, zy_lim = get_ext(x=x_data, y=y_data, x_lim=x_lim, y_lim=y_lim)

    # Check out limits
    shape_i = i_data.shape
    (shape_i_y, shape_i_x) = shape_i
    (v_lim_0, v_lim_1) = v_lim
    (i_lim_0, i_lim_1) = i_lim
    (didv_lim_0, didv_lim_1) = didv_lim

    print(shape_i_y, shape_i_x, v_data.shape)

    temp_i_data_pos = np.copy(i_data)
    temp_i_data_neg = np.copy(i_data)
    temp_didv_data_pos = np.copy(didv_data)
    temp_didv_data_neg = np.copy(didv_data)

    for iy in range(shape_i_y):
        for ix in range(shape_i_x):
            if v_lim_0 is None:
                v_lim_0 = np.nanmin(v_data)
            if v_lim_1 is None:
                v_lim_1 = np.nanmax(v_data)

            if v_data[ix] > v_lim_1 or v_data[ix] < v_lim_0:
                temp_i_data_pos[iy, ix] = np.nan
                temp_didv_data_pos[iy, ix] = np.nan

            if v_data[ix] < -v_lim_1 or v_data[ix] > -v_lim_0:
                temp_i_data_neg[iy, ix] = np.nan
                temp_didv_data_neg[iy, ix] = np.nan

            if i_lim_0 is None:
                i_lim_0 = np.nanmin(i_data)
            if i_lim_1 is None:
                i_lim_1 = np.nanmax(i_data)

            # if i_data[iy, ix] < i_lim_0 or np.isnan(i_data[iy, ix]):
            #     temp_i_data_pos[iy, ix] = np.nan
            # if i_data[iy, ix] > -i_lim_0 or np.isnan(i_data[iy, ix]):
            #     temp_i_data_neg[iy, ix] = np.nan

            # if i_data[iy, ix] > i_lim_1 or np.isnan(i_data[iy, ix]):
            #     temp_i_data_pos[iy, ix] = np.nan
            # if i_data[iy, ix] < -i_lim_1 or np.isnan(i_data[iy, ix]):
            #     temp_i_data_neg[iy, ix] = np.nan
        ...
        #     iy,
        #     i_data[iy, ix],
        #     temp_i_data_pos[iy, ix],
        #     temp_i_data_neg[iy, ix],
        #     i_lim,
        # )
    ...
    for index in indices:
        axs[1, 0].plot(
            v_data,
            temp_i_data_pos[index, :],
        )
        axs[1, 0].plot(-v_data, -temp_i_data_neg[index, :], "--")
        plt.plot(
            v_data,
            temp_i_data_pos[index, :],
        )
        plt.plot(-v_data, -temp_i_data_neg[index, :], "--")
        # plt.plot(v_data, didv_data[index, :])
        # plt.plot(-v_data, didv_data[index, :])
    plt.figure(20)
    plt.imshow(temp_i_data_pos, aspect="auto")
    plt.figure(21)
    plt.imshow(temp_i_data_neg, aspect="auto")
    plt.show()
    # plt.show()

    # if i_lim[0] is not None:
    #     temp_i_data = i_data[i_lim[0] < i_data]
    # if i_lim[1] is not None:
    #     temp_i_data = i_data[i_lim[1] > i_data]

    # plt.close("all")

    # for index in indices:
    #     axs[1, 0].plot(
    #         v_data[zero_v_lim:upper_v_lim],
    #         temp_i_data_pos[index, zero_v_lim:upper_v_lim],
    #     )
    #     axs[1, 0].plot(
    #         -v_data[lower_v_lim:zero_v_lim],
    #         temp_i_data_neg[index, lower_v_lim:zero_v_lim],
    #     )
    #     axs[2, 0].plot(
    #         v_data[zero_v_lim:upper_v_lim], didv_data[index, zero_v_lim:upper_v_lim]
    #     )
    #     axs[2, 0].plot(
    #         -v_data[lower_v_lim:zero_v_lim], didv_data[index, lower_v_lim:zero_v_lim]
    #     )
    temp_v_lim = axs[1, 0].get_xlim()
    temp_i_lim = list(axs[1, 0].get_ylim())
    temp_didv_lim = list(axs[2, 0].get_ylim())

    # print(temp_v_lim, temp_i_lim, temp_didv_lim)

    # for l_index, lim in enumerate(i_lim):
    #     if lim is not None:
    #         temp_i_lim[l_index] = lim

    # for l_index, lim in enumerate(didv_lim):
    #     if lim is not None:
    #         temp_didv_lim[l_index] = lim
    ...
    if n_show:
        if y_lim[0] is not None:
            y_indices_0 = np.argmin(np.abs(y_data - y_lim[0]))
        else:
            y_indices_0 = 0

        if y_lim[1] is not None:
            y_indices_1 = np.argmin(np.abs(y_data - y_lim[1]))
        else:
            y_indices_1 = -1
        axs[0, 1].plot(y_data[y_indices_0:y_indices_1], n_data[y_indices_0:y_indices_1])
    ...
    for j, index in enumerate(indices):
        ...
        ax_z.plot(
            np.array(x_lim), (y_data[index], y_data[index]), lw=2, color=COLORS[j + 1]
        )
    ax_i.plot(
        [1000, 1001],
        [1000, 1001],
    ...
    _ = ax_i.set_ylim(tuple(temp_i_lim))
    _ = ax_didv.set_xlim(temp_v_lim)
    _ = ax_didv.set_ylim(tuple(temp_didv_lim))
    ...
    if n_show:
        for j, index in enumerate(indices):
            ax_n.plot([y_data[index], y_data[index]], temp_n_lim, lw=2, color=COLORS[j])
        ax_n.plot(y_data, n_data, n_style, color=n_color, lw=n_lwms, ms=n_lwms)
